...
ESP32S3_BUILD_DIR = $(BUILD_BASE)/esp32s3_build
ESP32_MK_FLAGS = DISTRO=1 BUILD_BASE=$(ESP32S3_BUILD_DIR) -C $(TRONFERNO_MCU_ROOT)
...
ESP32S3_TEST_BUILD_DIR = $(BUILD_BASE)/esp32s3_test_build
ESP32S3_TEST_MK_FLAGS = DISTRO=1 BUILD_BASE=$(ESP32S3_TEST_BUILD_DIR) -C $(TRONFERNO_MCU_ROOT)
...
esp32s3: pre_esp32s3 main_esp32s3 post_esp32s3
esp32s3_test: pre_esp32s3 test_esp32s3
...
pre_esp32s3: co_master test_host
	cd $(TRONFERNO_MCU_ROOT) && git checkout --force $(GIT_BRANCH) && git pull && git clean -fd
	mkdir -p firmware/esp32s3
	make  $(ESP32_MK_FLAGS) esp32s3-clean
test_esp32s3:
	make $(ESP32_TEST_MK_FLAGS) esp32s2-test-clean esp32s3-test-build esp32s3-test-flash esp32s3-test-run
main_esp32s3:
	make -j  $(ESP32_MK_FLAGS) esp32s3-all
post_esp32s3: copy_docs
	cp -p $(ESP32S3_BUILD_DIR)/bootloader/bootloader.bin $(ESP32S3_BUILD_DIR)/tronferno-mcu.elf $ $(ESP32S3_BUILD_DIR)/tronferno-mcu.bin $(ESP32S3_BUILD_DIR)/ota_data_initial.bin ./firmware/esp32s3/
	cp -p $(ESP32S3_BUILD_DIR)/partition_table/partition-table.bin ./firmware/esp32s3/partitions.bin
